...
bool ros_ign_interfaces__msg__contacts__convert_from_py(PyObject * _pymsg, void * _ros_message)
{
  ...
  {  // contacts
    ...
    if (!ros_ign_interfaces__msg__Contact__Sequence__init(&(ros_message->contacts), size)) {
      PyErr_SetString(PyExc_RuntimeError, "unable to create ros_ign_interfaces__msg__Contact__Sequence ros_message");
      Py_DECREF(seq_field);
      Py_DECREF(field);
      return false;
    }
    ros_ign_interfaces__msg__Contact * dest = ros_message->contacts.data;
    for (Py_ssize_t i = 0; i < size; ++i) {
      if (!ros_ign_interfaces__msg__contact__convert_from_py(PySequence_Fast_GET_ITEM(seq_field, i), &dest[i])) {
        Py_DECREF(seq_field);
        Py_DECREF(field);
        return false;
      }
...
PyObject * ros_ign_interfaces__msg__contacts__convert_to_py(void * raw_ros_message)
{
  ...
  {  // contacts
    ...
    for (size_t i = 0; i < size; ++i) {
      ...
      PyObject * pyitem = ros_ign_interfaces__msg__contact__convert_to_py(item);
      if (!pyitem) {
        Py_DECREF(field);
        return NULL;
      }
